...
export class DatabaseStorage implements IStorage {
  ...
  async updateAgency(
    ...
    try {
      ...
      if (agencyData.latitude !== undefined) updates.latitude = agencyData.latitude;
      if (agencyData.longitude !== undefined) updates.longitude = agencyData.longitude;
  ...
  async updateClient(id: number, client: InsertClient): Promise<Client> {
    ...
    return await db.transaction(async (tx) => {
      ...
      // Drizzle does not reliably include these custom-typed columns in its
      // generated update, so patch them in the same transaction. PostgreSQL
      // needs an explicit array expression because the incoming value is a
      // JavaScript array.
      const tagsSql = Array.isArray(client.tags)
        ? sql`ARRAY[${sql.join(client.tags.map((tag) => sql`${tag}`), sql`, `)}]::text[]`
        : sql`NULL::text[]`;
      await tx.execute(sql`
        UPDATE clients
        SET
