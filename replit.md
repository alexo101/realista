...
- **Property Management**: Full CRUD operations for properties with multi-step creation workflow (5 steps), draft support after Step 2, progress stepper, advanced filtering, and integration with Barcelona's district and neighborhood data. Google Maps integration provides live address preview with geocoding, 200px compact map display, debounced API calls (500ms), proper memory cleanup, and real-time updates using useWatch from react-hook-form. Draft properties (isDraft=true, isActive=false) automatically open in multi-step creation flow when edited, allowing agents to continue where they left off. Published properties use simple edit form.
...
- **Client Relationship Management (CRM)**: Client profiles with status tracking, appointment scheduling, real-time conversational messaging, and lead/inquiry tracking with property visit request workflows.
    - **Client Status Tracking** (November 2025): 6-tier streamlined client lifecycle management with blue gradient visual indicators (Nuevo, Seguimiento, En visitas, Cerrando, Ganado, Perdido). Default status "Nuevo" auto-assigned on creation. Status field stored in `clients.status` column (TEXT, NOT NULL, default 'Nuevo'). Kanban view renamed to "Panel" for clarity.
    - **Contact History Timeline** (November 2025): Each client has a `contactHistory` JSONB field storing an array of timeline entries. Each entry includes id (UUID), status, timestamp (ISO string), and note text. Timeline UI appears in edit mode of AddClientModal, showing chronological notes with status badges, formatted timestamps (date-fns Spanish locale), and hover-to-delete functionality. Add note form includes status selector and textarea for capturing client interactions.
