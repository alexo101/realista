...
- **Agency Coordinates & Map View**: Agencies have `latitude`/`longitude` columns (added by migration `0006_agency_coordinates.sql`) so they can be plotted on the neighborhood-results page Agencias tab.
  - **Capture**: When an agency admin selects an address from `AddressAutocomplete` (Google Places), the new `onPlaceSelected` callback hands the lat/lng pair to `AgencyForm`, which sends it on POST/PATCH `/api/agencies` and persists it via `storage.createAgency` / `updateAgency`. Manually editing the address text clears the cached coords so they cannot drift from the displayed address.
  - **Mapa view on neighborhood results**: The Agencias tab now has a Lista/Mapa toggle (desktop) and a `FloatingMapButton` (mobile). In Mapa mode, `GoogleMapsAgenciesMap` plots each agency with a teal building-shaped pin (visually distinct from the property pins). Clicking a pin opens an InfoWindow with logo, name, rating + review count, and a "Ver perfil" button → `/agencias/<slug or id>`. Clicking another pin or anywhere on the map closes the open InfoWindow (single-open behavior matching the property map).
  - **Geocoding fallback**: Agencies whose `latitude`/`longitude` are still NULL are geocoded on the fly in the browser via `geocodeAddress` (the existing `/api/geocode` endpoint). The map shows a "Cargando ubicaciones…" pill while resolution is in flight.
  - **Backfill**: `tsx scripts/backfill-agency-coordinates.ts` geocodes existing agencies (defaults to only those missing coords; supports `--all`, `--agency-id=N`, `--dry-run`). Requires `GOOGLE_MAPS_API_KEY` and is rate-limited to 5 req/s.
